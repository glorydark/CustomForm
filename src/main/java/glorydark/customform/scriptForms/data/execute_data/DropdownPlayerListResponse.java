...
public class DropdownPlayerListResponse extends SimpleResponseExecuteData {
    ...
    public String replace(String text, Player player, boolean addQuotationMark, Object... params) {
        if (addQuotationMark) {
            ...
            return Api.strReplace(ready.replace("%content%", "\"" + params[1] + "\"").replace("%contentId%", String.valueOf(params[0])).replaceFirst("console#", "").replaceFirst("op#", ""), player);
        } else {
            String ready = text.replace("%player%", player.getName()).replace("%level%", player.getLevel().getName());
            return Api.strReplace(ready.replace("%content%", params[1].toString()).replace("%contentId%", String.valueOf(params[0])).replaceFirst("console#", "").replaceFirst("op#", ""), player);
        }
    }
}
